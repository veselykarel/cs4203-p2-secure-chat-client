...
class SecureChatClient:
    ...
    def send_secure_message(self, recipient_username: str, message: str):
        ...
        recipient_public_key = get_public_key(recipient_username)
        ...
        print(ciphertext)
        resp = requests.post(f"http://localhost:8000/chat/send/", json={
    ...
    def receive_secure_messages(self):
        ...
        resp = requests.get(f"http://localhost:8000/chat/receive/?msg_from={self.username}")
        if resp.status_code != 200:
    ...
    def _decrypt_secure_messages(self, messages):
        for message in messages:
            their_verify_key = get_verify_key(message['sender'])
...
def get_public_key(username) -> PublicKey:
    resp = requests.get(f"http://localhost:8000/identity/public/?username={username}")
    if resp.status_code == 200:
        return PublicKey(base64.b64decode(resp.text))
    else:
        raise Exception("failed to fetch public key")

def get_verify_key(username) -> VerifyKey:
    resp = requests.get(f"http://localhost:8000/identity/verify/?username={username}")
    if resp.status_code == 200:
